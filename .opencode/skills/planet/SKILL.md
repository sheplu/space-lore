...
Parse parameters if given (e.g. `type:oceanic`, `life:true`, `rings:true`), otherwise surprise the user.
...
5. Build the planet object: `name`, `description`, `tags`, `id`, `orbitIndex`, `orbitalDistanceAu`, `type`, `radiusEarth`, `gravityG`, `meanTempC`, `atmosphereDensity`, `moonCount`, `hasRings`, `life`. All stats inside the chosen type's taxonomy ranges; `life` not above its `lifeCeiling`; respect requested filters (`life:true` means at least microbial).
...
9. Report: planet name, type, id, orbit, one-line summary.
