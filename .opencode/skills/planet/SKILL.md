...
3. Choose the next free orbit: `orbitIndex` must be unique across ALL star-orbiting bodies in the system (planets, dwarf planets, asteroids, belts, comets — keep ascending); `orbitalDistanceAu` must increase with orbitIndex.
...
9. If user requested `moons:N`, generate N moons for this planet:
   - For each moon index 1..N:
     ...
     - Build moon object with stats from moon taxonomy (`planetId` must equal the parent planet's `id`, `orbitIndex` unique within the planet, plus type, radiusKm, gravityG, orbitalDistanceKm, hasAtmosphere)
